...
def sidebar():
    ...
    if "top_coins" in st.session_state and st.session_state.top_coins:
        ...
        selected_coin_name = st.sidebar.selectbox(
            ...
            index=list(coin_options.values()).index(st.session_state.selected_coin) if st.session_state.selected_coin in list(coin_options.values()) else 0
        )
...
def main_dashboard():
    ...
    with tab3:
        ...
        if "top_coins" in st.session_state and len(st.session_state.top_coins) > 1:
            ...
            top_5_coins = st.session_state.top_coins[:5]

            with st.spinner("Loading correlation data..."):
                for coin in top_5_coins:
                    coin_id = coin['id']
                    if coin_id in st.session_state.historical_data and st.session_state.time_period in st.session_state.historical_data[coin_id]:
                        historical_data_dict[coin_id] = st.session_state.historical_data[coin_id][st.session_state.time_period]
                    else:
